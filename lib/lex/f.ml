...
open Syntax.Tiger
open Syntax.Tiger.MenhirInterpreter
...
module Lexer : sig
  ...
  let initialize lexbuf = more := lexer_lexbuf_to_supplier Tiger.read lexbuf

  type t = int

  let start = 0
  let get pos = List.nth !buffer (!size - pos)
  let token_of_ptoken (p, _, _) = p
  let current_position_of_ptoken (_, start, stop) = Position.lex_join start stop
  let current_position pos = current_position_of_ptoken (get pos)
  let get' pos = token_of_ptoken (get pos)
  ...
  let skip_until_before pred pos =
    let rec aux pos =
      ...
      if token = EOF then pos else if pred token then pos - 1 else aux (snd (next pos))
    in
    aux pos
  ;;
end
